...
(function() {
  var DownCourse, GetCourse, async, checkPage, cheerio, fs, request,
  ...
  async.auto({
    getPage: function(cb) {
      ...
      return page.getPageUrl(function(err) {
        if (err) {
          return cb(err);
        }
        return cb(null, page.nextUrl);
      });
    },
    downUrlList: [
      'getPage', function(cb, result) {
        ...
        return async.eachSeries(urlArr, function(item, callback) {
          var down;
          down = new GetCourse(item);
          return down.getCourseUrl(function(err1, courseArr) {
            if (err1) {
